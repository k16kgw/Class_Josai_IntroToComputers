...
- **restoring（復元）法**
  ...
  2. (R\leftarrow R - D)
  3. (R\ge 0) なら 商_bit=1（そのまま）／ (R<0) なら 商_bit=0 & (R\leftarrow R + D)（**復元**）

- **non-restoring（非復元）法**
  “引けなかったら次サイクルで加える”に切り替え，**復元を省く**（状態で加減を選択）

- **構成（データパス）**：

  - SR_N（被除数），REG_D（除数），REG_Q（商），REG_R（余り），減算器/加算器，共通制御CNT

- **ブロック図**：`/contents/figs/12/div_sequential.png`

- **特性**：面積小，レイテンシ ≈ **nサイクル**（±α）．

**手を動かす2（5分）｜4ビット restoring**
N=1101(=13), D=0011(=3)．各サイクルで (R,Nの次ビット, 引減, 商ビット) を追跡し，Q と R を求めよ．
（答え：Q=0101(=5), R=0000）
<!-- 
### 高速化の入口
...
- 逐次型：**シフト＋加減算＋レジスタ＋カウンタ＋制御**の組で作る（理解の母体，面積小）．
